      ...
      <el-dialog title="摘要 分类 标签"
        ...
        <el-form :model="articleForm" ref="articleForm" :rules="rules">
          ...
          <el-form-item label="文章分类" prop="category">
            <el-select v-model="articleForm.category" value-key="id" placeholder="请选择文章分类" :allow-create="true" filterable>
              <el-option v-for="c in categorys" :key="c.categoryname" :label="c.categoryname" :value="c.categoryname" ></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item>
              <el-radio v-model="articleForm.is_public" :label="1">公开</el-radio>
              <el-radio v-model="articleForm.is_public" :label="0">私密</el-radio>
          </el-form-item>
        </el-form>
...
<script>
  ...
  import {publishArticle,viewArticle,updateArticle} from '@/api/article'
        ...
        this.$refs[articleForm].validate((valid) => {
          if (valid) {
            ...
            let article = {
              ...
              category_name: this.articleForm.category,
              tag_name: this.articleForm.tags,
              content: this.articleForm.editor.value,
              content_HTML: this.articleForm.editor.ref.d_render,
              is_public:this.articleForm.is_public,
            }
            ...
            if(article.id==""){
              publishArticle(article).then((data) => {
                ...
                that.$router.push({path: `/view/${data.id}`})
              ...
              updateArticle(article).then((data) => {
                ...
                that.$router.push({path: `/view/${this.$route.params.id}`})
